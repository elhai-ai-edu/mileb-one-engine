...
```json
{
  ...
  "activityBank": [],
  "courseTopology": {
    "source": "moodle | link_hierarchy | sequence_file",
    "stages": [],
    "actions": [],
    "gates": [],
    "sequenceLogic": {}
...
## 4.1 Course Topology — מקור מבנה הקורס

`courseTopology` הוא מקור קלט נפרד שמאפשר למנוע **לחלץ רצף פדגוגי מתוך מבנה קורס קיים**, גם כאשר אין lesson plan מפורש.

### מקורות תמיכה

המנוע צריך לתמוך בחילוץ topology מ:

| מקור | תיאור |
|------|-------|
| **Moodle structure** | מודולים, נושאים, יחידות ותתי-יחידות כפי שמוגדרים ב-Moodle |
| **Link hierarchies** | רשימת קישורים עם מבנה היררכי (parent / child) |
| **Existing course sequences** | רצפים מוגדרים-מראש של שיעורים, ספרינטים או יחידות |

### מה מחלצים מה-topology

```json
{
  "stages": [
    { "id": "s1", "label": "פתיחת נושא", "source_unit": "unit_3" }
  ],
  "actions": [
    { "stage_id": "s1", "type": "read", "resource_ref": "res_intro" }
  ],
  "gates": [
    { "after_stage": "s1", "condition": "completion_required" }
  ],
  "sequenceLogic": {
    "type": "linear | branching | gated",
    "entry_point": "s1",
    "exit_point": "s_last"
  }
}
...
- `courseTopology` משמש כ-**fallback** כאשר אין `lessonPlan` מפורש.
- כאשר קיים `lessonPlan`, ה-topology יכול לשמש לאימות ולהשלמת פערים.
- ה-topology אינו קובע תוכן — הוא קובע **סדר וגבולות**.

### החלטה מחייבת #3

אם `courseTopology` קיים אך `lessonPlan` חסר, המנוע ישתמש ב-topology כשלד בסיסי ויסמן זאת ב-build report כ-`source: topology_fallback`.
